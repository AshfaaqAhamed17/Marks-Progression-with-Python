...
def PROCESS():
    ...
    #starts printing vertical histogram
    print("\n""Progress  Trailing  Retreiver  Excluded")

    max_num=max(Progress, Trailing, Retreiver, Excluded)#check for maximum rows to be printed

    for i in range (max_num):
        if Progress>0:
            print("    *    ",end='')
            Progress-=1 #number of * to be printed is decreased by 1
        else:
            print("         ",end='')

        if Trailing>0:
            print("    *    ",end='')
            Trailing-=1 #number of * to be printed is decreased by 1
        else:
            print("         ",end='')
            
        if Retreiver>0:
            print("      *      ",end='')
            Retreiver-=1 #number of * to be printed is decreased by 1
        else:
            print("             ",end='')
            
        if Excluded>0:
            print("     *     ",end='')
            Excluded-=1 #number of * to be printed is decreased by 1
        else:
            print("           ",end='')
        print('')
